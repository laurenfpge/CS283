...
int exactString(char* str, cmd_buff_t *cmd){
	//printf("in exactString, str is: |%s|\n", str);
	...
	while (*str != '"'){
		*temp = *str;
	//	printf("temp %c, str %c, quotelen %d\n", *temp, *str, quotelen);
		temp++;
		str++;
		quotelen++;
	}
	...
	*temp = '\0';
	...
	temp = tempstart;
	//printf("tempstart: |%s|, quotelen: %d, help: %d\n", temp, quotelen, help);
...
int exec_local_cmd_loop()
{
	...
	//		printf("\ncurrent spctoken: |%s|, starts at index: %d, cmd.argc: %d\n", spctoken, index, cmd.argc);
...
//				index++;
			...
			//	printf("index in quotes: %d\n", index);
			//	printf("rest of string quotes: |%s|\n", &cmd._cmd_buffer[index]);
			...
			//	printf("index at add: %d\n", index);
			//	printf("rest of string add: |%s|\n", &cmd._cmd_buffer[index]);
			}
