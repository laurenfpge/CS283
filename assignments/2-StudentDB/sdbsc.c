...
int get_student(int fd, int id, student_t *s)
{
	...
	ssize_t numread = read(fd, s, STUDENT_RECORD_SIZE);
	if (numread < 0){
		return ERR_DB_FILE;
	} else if (numread != STUDENT_RECORD_SIZE){
		return SRCH_NOT_FOUND;
	}

	char allzeros[STUDENT_RECORD_SIZE];
	memset(allzeros, 0, STUDENT_RECORD_SIZE);

	if (memcmp(s, allzeros, STUDENT_RECORD_SIZE) == 0){
		return SRCH_NOT_FOUND;
	}
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
	...
	student_t temp;
    int findExisting = get_student(fd, id, &temp);
	
	if (findExisting == NO_ERROR){ //student exists
		printf(M_ERR_DB_ADD_DUP, id);
		return ERR_DB_OP;
	} else if (findExisting == ERR_DB_FILE){ //file issue
		printf(M_ERR_DB_READ);
		return ERR_DB_FILE;
	} //else SRCH_NOT_FOUND yay!

	int position = (off_t)id*STUDENT_RECORD_SIZE;
	...
	student_t new;
	memset(&new, 0, STUDENT_RECORD_SIZE);
	new.id = id;
	new.gpa = gpa;
	strncpy(new.fname, fname, sizeof(new.fname)-1);
	strncpy(new.lname, lname, sizeof(new.lname)-1);

	ssize_t numwrite = write(fd, &new, STUDENT_RECORD_SIZE);
	if ((numwrite == -1) || (numwrite != STUDENT_RECORD_SIZE)){
...
{
	student_t temp; //after check, temp is at id position
    int findExisting = get_student(fd, id, &temp);
	
	if (findExisting == ERR_DB_FILE){ //file issue
		printf(M_ERR_DB_READ);
		return ERR_DB_FILE;
	} else if (findExisting == SRCH_NOT_FOUND) {
		printf(M_STD_NOT_FND_MSG, id);
		return ERR_DB_OP;
	} //else NO_ERROR, student exists
	...
	ssize_t numwrite = write(fd, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE);
	if ((numwrite == -1) || (numwrite != STUDENT_RECORD_SIZE)){
		printf(M_ERR_DB_WRITE);
		return ERR_DB_FILE;
	}
...
int count_db_records(int fd)
{
	...
	char justzeros[STUDENT_RECORD_SIZE];
	memset(justzeros, 0, STUDENT_RECORD_SIZE);
	
	while(currstudent != MAX_STD_ID){		
    	offset = currstudent*STUDENT_RECORD_SIZE; 
		newpos = lseek(fd, offset, SEEK_SET);
    	...
    	numread = read(fd, &temp, STUDENT_RECORD_SIZE); 
		if (numread < 0){
			printf(M_ERR_DB_READ);
			return ERR_DB_FILE;
		}
	
		if (memcmp(justzeros, &temp, STUDENT_RECORD_SIZE) != 0){
			numstudent++;
		}
    ...
    if (numstudent == 0){
		...
		printf(M_DB_RECORD_CNT, numstudent);
		return numstudent;
	}
}
...
int print_db(int fd)
{
	...
	while(currstudent != MAX_STD_ID){		
    	...
    	numread = read(fd, &student, STUDENT_RECORD_SIZE); 
		if (numread < 0){
			printf(M_ERR_DB_READ);
			return ERR_DB_FILE;
		}
	
		if (memcmp(justzeros, &student, STUDENT_RECORD_SIZE) != 0){
			numstudent++;

			if (numstudent == 1){ //on first row
